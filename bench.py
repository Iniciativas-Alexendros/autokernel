...
def gen_rotary_embedding_inputs(
    size: dict, dtype: torch.dtype, device: str, seed: int = 42
) -> dict:
    torch.manual_seed(seed)
    batch, heads, seq_len, head_dim = (
...
def _ref_elementwise(inputs: dict) -> torch.Tensor:
    import reference
...
KERNEL_CONFIGS: Dict[str, Dict[str, Any]] = {
    ...
    "matmul": {
        ...
        "bytes_fn": lambda s, dt: (
            (s["M"] * s["K"] + s["K"] * s["N"] + s["M"] * s["N"]) * _dtype_bytes(dt)
        ),
    ...
    "elementwise": {
        ...
        "flops_fn": lambda s: 3 * s["N"],  # mul + add + silu
        "bytes_fn": lambda s, dt: 3 * s["N"] * _dtype_bytes(dt),  # read x, write silu*x
        "input_generator": gen_elementwise_inputs,
        "reference_fn": _ref_elementwise,
        "edge_sizes": [
            ("edge_1023", {"N": 1023}),
            ("edge_4097", {"N": 4097}),
        ],
    ...
    "rmsnorm": {
        ...
        "bytes_fn": lambda s, dt: (
            (2 * s["M"] * s["N"] + s["N"]) * torch.tensor([], dtype=dt).element_size()
        ),
    ...
    "reduce": {
        ...
        "bytes_fn": lambda s, dt: (
            (s["M"] * s["N"] + s["M"]) * torch.tensor([], dtype=dt).element_size()
        ),
...
def _compare(
    ...
    return {
        ...
        "reason": ""
        if match
        else f"max_abs_error={max_abs:.6e} exceeds tol(atol={atol}, rtol={rtol})",
        ...
        for dtype in dtypes:
            ...
            try:
                ...
                if not cmp["match"]:
                    ...
                    print(
                        f"  PASS: {label} {dtype} (max_err={cmp['max_abs_error']:.2e}, within_tol={cmp['pct_within_tol']:.1f}%)"
                    )
...
def run_performance(
    kernel_fn: Callable, config: dict, gpu: GPUSpec, sizes_filter: str = "all"
) -> dict:
    ...
    for label, sz in bench_sizes:
        ...
        try:
            ...
            if arithmetic_intensity < ridge_point:
                ...
                pct_peak_compute = (
                    (throughput_tflops / gpu.peak_tflops_fp16 * 100.0)
                    if gpu.peak_tflops_fp16 > 0
                    else 0.0
                )
            else:
            ...
            print(
                f"    kernel: {kernel_us:.2f} us | pytorch: {ref_us:.2f} us | "
