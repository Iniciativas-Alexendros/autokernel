...
def _prepare_model_and_input(
    ...
    for attempt_batch in [original_batch, max(1, original_batch // 2), 1]:
        ...
        try:
            ...
            print(f"  OOM with batch_size={attempt_batch}, trying smaller...")
            continue
        except Exception as e:
...
def profile_model(
    ...
    for evt in key_averages:
        ...
        cuda_time_us = getattr(evt, "self_device_time_total", None) or getattr(
...
def print_report(
    ...
    print(
        f"  Total GPU time: {report['total_gpu_time_ms']:.3f} ms "
    ...
    print(f"  Top-5 kernels account for {summary['top5_pct']:.1f}% of total GPU time.")
...
def main() -> int:
    ...
    # Profile
    print(f"Profiling... ({WARMUP_ITERS} warmup + {PROFILE_ITERS} measured iterations)")
    try:
        ...
        torch.cuda.empty_cache()
        print("ERROR: GPU out of memory during profiling. Try a smaller --input-shape.")
        return 1
    except Exception as e:
    ...
    if extras:
        ...
        if "memory_snapshot_path" in extras:
            print(
                f"    - Analyze with mosaic:   python -m torch.cuda.memory._snapshot {extras['memory_snapshot_path']}"
            )
        if "compile_log_path" in extras:
