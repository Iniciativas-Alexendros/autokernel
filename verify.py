...
def load_model_from_module(
    module_name: str, class_name: str, pretrained: Optional[str] = None, **kwargs
) -> nn.Module:
...
class OptimizedModelContext:
    ...
    def _apply_replacement(self, repl: KernelReplacement) -> int:
        ...
        elif repl.kernel_type == "flash_attention":
            count = self._replace_flash_attention(repl)
        else:
            print(
                f"  NOTE: No replacement strategy for kernel type '{repl.kernel_type}'. "
                f"Skipping. (Supported: matmul, layernorm, rmsnorm, flash_attention)"
            )
    ...
    def _replace_rmsnorm_modules(self, repl: KernelReplacement) -> int:
        ...
        for name, module in list(self.model.named_modules()):
            ...
            is_rmsnorm = cls_name in rmsnorm_names or (
    ...
    def _replace_flash_attention(self, repl: KernelReplacement) -> int:
        """
        ...
        def patched_sdpa(q, k, v, **kwargs):
            # Our optimized kernel signature: kernel_fn(Q, K, V, causal=True, sm_scale=None)
            # F.scaled_device_attention passes is_causal, scale, etc.
            causal = kwargs.get("is_causal", False)
            sm_scale = kwargs.get("scale", None)
            # Our kernel expects (batch, heads, seq, dim) which matches q, k, v
            return optimized_fn(q, k, v, causal=causal, sm_scale=sm_scale)
...
def main() -> None:
    ...
    for r in replacements:
        print(
            f"    {r.kernel_type} (rank {r.rank}): speedup={r.speedup:.1f}x -> {r.optimized_path}"
        )
    print()
